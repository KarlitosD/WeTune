...
function CreatePlaylistModal(props: { addPlaylist: (playlist: Playlist) => void }) {
    ...
    return (
        <>
            <button class="cursor-pointer outline-hidden" onClick={() => $dialog.showModal()}>
                <div class={`${playlistCardSize} rounded-sm flex justify-center items-center text-white bg-neutral`}>
                    <IconMusic size={48} />
                </div>
            ...
            <dialog class="modal" ref={$dialog}>
                <div class="modal-box">
                    ...
                    <form class="mx-auto flex justify-center gap-2 w-full sm:w-4/5 " onSubmit={importPlaylist}>
                        <input type="url" name="url" class="input w-full max-w-64" placeholder="Youtube Playlist URL" />
                        <button class="w-21.5 btn btn-neutral uppercase">{LL().IMPORT()}</button>
                    </form>
                    ...
                    <form onSubmit={createPlaylist} class="mx-auto flex justify-center gap-2 w-full sm:w-4/5">
                        <input type="text" name="title" class="input w-full max-w-64" placeholder={LL().NAME_PLAYLIST()} /> 
                        <button class="w-21.5 btn btn-neutral uppercase">{LL().ADD()}</button>
                    </form>
                </div>
...
function PlaylistCard(props: { playlist: Playlist }) {
    ...
    return (
        <a href={`/playlist/${props.playlist.id}`}>
            <div class={`${playlistCardSize} flex rounded-sm overflow-hidden`}>
...
function PlaylistWithoutImage() {
    return (
        <div class="w-full h-full rounded-sm bg-neutral flex justify-center items-center">
            <IconMusic size={48} />
        </div>
    )
}